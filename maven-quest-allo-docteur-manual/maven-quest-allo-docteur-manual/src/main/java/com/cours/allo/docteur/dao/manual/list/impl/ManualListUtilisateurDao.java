...
public class ManualListUtilisateurDao extends AbstractListDao<Utilisateur> implements IUtilisateurDao {
    ...
    public Utilisateur createUtilisateur(Utilisateur user) {
    	try {
    		List<Adresse> lst_adresse =  AbstractDaoFactory.getFactory(AbstractDaoFactory.FactoryDaoType.MANUAL_LIST_DAO_FACTORY).getAdresseDao().findAllAdresses();    		
    		user.setDateCreation(new Date());	            
     		user.setDateModification(new Date());
     		user.setVersion(1);
     		for (int j = 0; j < lst_adresse.size(); j++) {            			 
     			Adresse a = lst_adresse.get(j);
     			AbstractDaoFactory.getFactory(AbstractDaoFactory.FactoryDaoType.MANUAL_LIST_DAO_FACTORY).getAdresseDao().updateAdresse(a); 
     		}     		
    ...
    public boolean deleteUtilisateur(Utilisateur user) throws CustomException{
    	if (DataSource.getInstance().getUtilisateursListDataSource().remove(user)) {
			return true;
		} else {
			throw new CustomException("L'utilisateur portant l'identifiant " + user.getIdentifiant() + " n'existe pas",1);
		}
	}
